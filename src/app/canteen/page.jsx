  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setErrorMessage(data.error || 'Error submitting the form. Please try again.');
      }
    } catch (error) {
      console.log(error);
      setStatus('error');
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-green-100 to-blue-100 py-12 px-4 sm:px-6 lg:px-8'>
      <div className='max-w-5xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden'>
        ...
        {status === 'error' && <p className='text-red-600 text-center mt-4 pb-6'>{errorMessage || 'Error submitting the form. Please try again.'}</p>}
      </div>
    </div>
  );
}
